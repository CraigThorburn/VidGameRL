...
python acousticgame_run_abx_craig.py $params -run_num=$i -layer=-1 -pretrain=true || exit
...
if [ $stage -le 6 ]; then
echo "starting abx for both of last layers craig"
python acousticgame_run_abx_craig.py $params -run_num=$i -layer=-1 -pretrain=false || exit
