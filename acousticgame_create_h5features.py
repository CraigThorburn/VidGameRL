...
data = SpeechDataLoader(ROOT + VALIDATION_SEGMENTS_FILE + '.txt', ROOT + PHONES_FILE + '.txt', ROOT + VALIDATION_ALIGNMENTS_FILE + '.txt', ROOT + WAVS_FOLDER , device) # Copied from pretrain_validation
...
phoneme_classifier = PhonemeConvNN(KERNEL, STRIDE, w, h, n_outputs).to(device)
...
with h5f.Writer(FEATURES_OUTPATH + TRAIN_MODELNAME + ".features") as writer:
