...
python acousticgame_pretrain_network.py $params -run_num=$i
echo "pretraining complete"
fi

if [ $stage -le 1 ]; then
echo "starting pretrain validation"
python acousticgame_pretrain_validation.py $params -run_num=$i
echo "pretrain validation complete"
fi

if [ $stage -le 2 ]; then
echo "calculating fischer coefficients"
python acousticgame_calculate_ewc_coeffs.py $params -run_num=$i
fi


if [ $stage -le 3 ]; then
echo "starting training from pretrained model"
python acousticgame_train.py $params -run_num=$i
