...
class DQN_NN_conv_pretrain_phonelayer_branching(nn.Module):

    def __init__(self, h, w, inputs, outputs, extranodes=2, kernel=5, sstride=2, layers=[16, 32, 32, 20],
                 freeze_convolution=False, include_midlayer=True, n_phone_layer=39, freeze_layer=0):
        super(DQN_NN_conv_pretrain_phonelayer_extranodes, self).__init__()
        ...
        if self.include_midlayer:
            self.head1 = nn.Linear(extranodes + inputs, self.mid_size)
            self.head2 = nn.Linear(self.mid_size, outputs)
        else:
            self.head = nn.Linear(extranodes + inputs, outputs)
    ...
    def forward(self, x_aud, x_loc):
        ...
        x = torch.cat((x_aud_extra, x_loc), 1)
