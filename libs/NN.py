...
class PhonemeConvNN(nn.Module):
    ...
    def get_out_from_layer(self, x, layer, stacked_out=None, only_extranodes=None):
...
class SupervisedConvNN(nn.Module):

    def __init__(self, kernel, sstride, w, h, n_outputs, layers = [32, 32, 32, 20], window_length = 100):
        super(SupervisedConvNN, self).__init__()
        self.conv1channels, self.conv2channels, self.conv3channels, self.mid_size = layers
        # Number of Linear input connections depends on output of conv2d layers
        # and therefore the input image size, so compute it.
        def conv2d_size_out(size, kernel_size=kernel, stride=sstride):
        ...
        self.linear_input_size = convw * convh * self.conv2channels
...
class PhonemeConvNN_extranodes(nn.Module):
    ...
    def forward(self, x, concat=False):
        ...
        if not concat:
            ...
            return torch.cat((x_orig, x_extra), 1)

    def get_out_from_layer(self, x, layer, stacked_out=False, only_extranodes=False):
