...
for trial in [1, 2]:
    for i_batch in range(n_batches):
        ...
        if OUT_LAYER != -1:
            ...
            predictionsa, predictionsa_extra = phoneme_classifier(
                batcha.reshape(batcha.size()[0], 1, batcha.size()[1], batcha.size()[2]))
            predictionsb, predictionsb_extra = phoneme_classifier(
                batchb.reshape(batchb.size()[0], 1, batchb.size()[1], batchb.size()[2]))

            predictionsx, predictionsx_extra = phoneme_classifier(
                batchx.reshape(batchx.size()[0], 1, batchx.size()[1], batchx.size()[2]))

        if len(predictionsx.size())==2:
            predictionsx = torch.cat((predictionsx, predictionsx_extra),1)
            predictionsa = torch.cat((predictionsa, predictionsa_extra), 1)
            predictionsb = torch.cat((predictionsb, predictionsb_extra), 1)
            pass
