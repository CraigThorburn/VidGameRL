...
python fullsupervision_train.py $params -run_num=$i || exit
...
if [ $stage -le 5 ]; then
echo "starting abx for both of last layer"
python fullsupervision_run_abx.py $params -run_num=$i -layer=-1 -pretrain=false || exit
